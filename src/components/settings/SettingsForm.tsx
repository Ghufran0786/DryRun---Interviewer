  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <fieldset className="space-y-4 rounded-[6px] border border-[#E5E5E5] bg-[#FAFAFA] p-4">
        ...
        {voiceResult ? (
          <p className="text-xs text-[#0A0A0A]">{voiceResult}</p>
        ) : null}
      </fieldset>
      <FieldInput
      ...
      />
      {error ? (
        <p className="text-sm font-medium text-[#0A0A0A]">{error}</p>
      ) : null}
      {saved ? (
        <p className="text-sm text-[#6B6B6B]">Settings saved.</p>
      ) : null}
      {modelResults.length > 0 ? (
        <div className="space-y-2 rounded-[6px] border border-[#E5E5E5] bg-[#FAFAFA] p-3">
          {modelResults.map((result) => (
            <p key={result.role} className="text-xs text-[#0A0A0A]">
